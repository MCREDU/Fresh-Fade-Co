...
function validateForm() 
{
    ...
    if(id_tab == "client_tab")
    {
        ...
        var is_existing_customer = $('#existing_customer').is(':checked');

        // If existing customer, only validate email
        if(is_existing_customer) {
            if(!ValidateEmail(client_email))
            {
                $('#client_email').css("border", "2px solid #dc3545");
                $('#client_email').next('.error-message').css("display", "block");
                valid = false;
            }
            else
            {
                $('#client_email').css("border", "0px");
                $('#client_email').next('.error-message').css("display", "none");
            }
        } else {
            // New customer: validate all fields
            if($.trim(client_f_name) == "")
            {
                $('#client_first_name').css("border", "2px solid #dc3545");
                $('#client_first_name').next('.error-message').css("display", "block");
                valid = false;
            }
            else
            {
                $('#client_first_name').css("border", "0px");
                $('#client_first_name').next('.error-message').css("display", "none");

                if($.trim(client_l_name) == "")
                {
                    $('#client_last_name').css("border", "2px solid #dc3545");
                    $('#client_last_name').next('.error-message').css("display", "block");
                    valid = false;
                }
                else
                {
                    $('#client_last_name').css("border", "0px");
                    $('#client_last_name').next('.error-message').css("display", "none");
                    ...
                    else
                    {
                        ...
                        if(!phonenumber(client_phone_number))
                        {
                            ...
                            $('#client_phone_number').next('.error-message').css("display", "block");
                            valid = false;
                        }
                        else
                        {
                            $('#client_phone_number').css("border", "0px");
                            $('#client_phone_number').next('.error-message').css("display", "none");
                        }
                    }
                }
            }
